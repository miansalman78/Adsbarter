...
const Proposals = ({ navigation, userRole }) => {
  const [showSortDropdown, setShowSortDropdown] = useState(false);
  const [selectedSort, setSelectedSort] = useState('Best Match');
  const [showFilterDropdown, setShowFilterDropdown] = useState(false);
  const [activeTab, setActiveTab] = useState('Proposals'); // Track active tab for bottom navigation
  // Only show bottom tabs for Brand role, not Creator role
  const isBrandRole = userRole === 'Brand' || userRole === 'brand';
